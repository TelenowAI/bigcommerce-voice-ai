...
// `X-API-Key: vai_live_...` header. Construct one client per store with that
// store's key (see settings.js / store.js for where the key lives).
//
...
/** Hook `source` tag — identifies this integration on the Telenow side. */
export const HOOK_SOURCE = 'bigcommerce';
...
export class TelenowClient {
   ...
   * NOTE: this endpoint lives under /api/sessions (NOT /api/v1).
   * @param {object} args
   ...
   * @param {string} [args.identifier]    Your correlation id (order/customer id).
   ...
   * @param {string} [args.source='bigcommerce']
