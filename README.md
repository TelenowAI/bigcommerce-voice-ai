...
Drop-in copy for the **BigCommerce App Marketplace** listing. Set **Pricing → Free**. Because the app connects to a paid external service, the listing must disclose it (a common review-rejection reason if hidden).

- **App name:** Telenow Voice AI — Automated Customer Calls
- **Category (suggested):** Sales & Conversion → Order Management / Customer Support (cross-list under Marketing if a second category is allowed).

### Summary (one line)

> AI voice agent that automatically phones your shoppers to confirm COD orders, recover abandoned carts, give delivery updates, call new leads back, and win lapsed customers back — then writes every call outcome onto the order.

### Tagline (short)

> Automated AI phone calls for COD confirmation, abandoned-cart recovery, order updates, instant lead callback, and win-back.

### Description (benefit-led)

> **Turn store events into outbound phone calls — automatically.** Telenow Voice AI connects your BigCommerce store to your Telenow voice agents so a natural-sounding AI calls the right shopper at the right moment, in English, Hindi, and other Indian and global languages, and records the result straight onto the order. No call-center staff, no glue code.
>
> **Confirm cash-on-delivery orders and cut RTO.** The moment a COD order comes in, the AI calls the buyer to confirm the order and address before you ship, helping reduce return-to-origin (RTO) losses and fake or abandoned COD orders. The outcome — confirmed, cancelled, or no-response — is written to a staff note and an order metafield so your fulfillment team can act before dispatch.
>
> **Recover abandoned carts with a real phone call.** When a shopper leaves items behind, Telenow follows up with a friendly recovery call and the link back to checkout — a higher-touch nudge than yet another email.
>
> **Keep buyers informed and bring them back.** Place automatic order-confirmation and delivery/shipping-update calls (so the AI can answer "where is my order?"), retry failed or refused deliveries (NDR), run win-back / re-engagement calls to lapsed customers with an optional discount, and collect review, feedback, and NPS responses by voice.
>
> **Call every new lead in seconds.** New-customer sign-ups trigger an instant speed-to-lead callback, and every lead and call result shows up in the in-app dashboard.
>
> **Two-way and on-brand.** During a call the AI can look up live order, product, and customer details to answer questions, and it uses the Telenow agents you've already built — your script, your voice, your languages.
>
> **Free app — requires a Telenow account.** This app is free to install. It connects to Telenow (https://telenow.ai), a separate AI voice-calling service that requires its own account. Telenow charges for call usage on its own platform per its pricing (https://telenow.ai/#pricing); these charges are not processed by BigCommerce. The app sends your shopper's phone number and the order context you enable to Telenow to place calls — no payment card data is sent. Telenow's Terms (https://telenow.ai/terms) and Privacy Policy (https://telenow.ai/privacy) apply.

### Keywords / tags

`abandoned cart recovery`, `cash on delivery confirmation`, `COD verification`, `RTO reduction`, `order confirmation call`, `delivery updates`, `shipping notifications`, `failed delivery retry`, `NDR`, `win-back`, `customer re-engagement`, `lead callback`, `speed to lead`, `AI voice agent`, `automated phone calls`, `outbound calling`, `voice automation`, `multilingual voice AI`, `Hindi voice agent`, `where is my order`, `review and feedback calls`, `NPS calls`, `customer retention`

### Pricing & disclosure

- **Pricing:** Free to install. Requires a Telenow account; Telenow bills for call usage on its own platform (no charges through BigCommerce).
- **Privacy / data handling (for the listing's data section):** sends shopper phone + order context to Telenow to place calls; stores call metadata (session→order) and new-customer leads (name/email/phone) for the dashboard and write-back; purges all of it on uninstall. Voice recordings/transcripts are held by Telenow.
- **Support:** support@telenow.ai · Privacy: https://telenow.ai/privacy · Terms: https://telenow.ai/terms · Pricing: https://telenow.ai/#pricing
